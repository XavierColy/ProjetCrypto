...
public class Client {
	...
	public static void chiffrerPieceJointe(String path, String user,PublicParameter PP) {
	...
	public static void dechiffrerPieceJointe(String path,Pairing pairing, Element secretKey) {
		
		if((UtilsForEncryptSystem.getFileExtension(path)).equals("encrypt")) {
			String path2 = UtilsForEncryptSystem.removeLastNchars(path.toString(), 8);
			...
			try {
				UtilsForEncryptSystem.produireFichierDechiffrer(path2, byteListe);
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
